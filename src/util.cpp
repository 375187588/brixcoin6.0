...
const char * const BITCOIN_CONF_FILENAME = "brixcoin.conf";
const char * const BITCOIN_PID_FILENAME = "brixcoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "brixcoin" is a composite category enabling all Brixcoin-related debug output
                if(ptrCategory->count(std::string("brixcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BrixcoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BrixcoinCore
    // Mac: ~/Library/Application Support/BrixcoinCore
    // Unix: ~/.brixcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BrixcoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BrixcoinCore";
#else
    // Unix
    return pathRet / ".brixcoincore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty brixcoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override brixcoin.conf
