...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in brixcoin.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("brixcoin.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Brixcoin features */
    { "brixcoin",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "brixcoin",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "brixcoin",               "gobject",                &gobject,                true,  {} },
    { "brixcoin",               "voteraw",                &voteraw,                true,  {} },
